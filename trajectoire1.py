...
class CarlaLidarPublisher(Node):
    def __init__(self):
        ...
        self.pose_publisher = self.create_publisher(Float32MultiArray, 'vehicle/pose', 10)
        self.odom_pub = self.create_publisher(Odometry, '/vehicle/odometry', 10)
        self.compass_pub = self.create_publisher(Float32, '/vehicle/compass_heading', 10)
        ...
        '''self.file = open('lidar_points.csv', 'w', newline='')
        self.writer = csv.writer(self.file)
        self.writer.writerow(['x', 'y', 'z', 'intensity'])  
    ...
    def init_lidar(self, vehicle):
        ...
        lidar_bp.set_attribute('rotation_frequency', '10') # 10
        lidar_bp.set_attribute('points_per_second', '300000') # 300000
    ...
    def process_lidar(self, data):
        ...
        # Publier position et orientation
        transform = self.vehicle.get_transform()
        x = transform.location.x
        y = transform.location.y
        yaw = transform.rotation.yaw  # en degrés

        pose_msg = Float32MultiArray()
        pose_msg.data = [x, y, yaw]
    ...
    def drive_to_target(self, target_location, stop_distance=25, speed=0.4):
       ...
       while rclpy.ok():
            ...
            odom.pose.pose.position.z = float(tf.location.z)
            odom.pose.pose.orientation = self.yaw_to_quat(yaw_deg)
            self.odom_pub.publish(odom)

            # Publier Compass (allocentrique)
            heading_rad = math.radians(90.0 - yaw_deg)   # 0 = Nord
            heading_rad = (heading_rad + math.pi) % (2 * math.pi) - math.pi
            self.compass_pub.publish(Float32(data=heading_rad))
   
            time.sleep(0.05)

    def create_pointcloud2_msg(self, points, ts):
        pts = points if isinstance(points, np.ndarray) else np.frombuffer(points, np.float32)
        n = pts.size // 4
        pts = pts[:n*4]  # sécurité
        msg = PointCloud2()
        msg.header.stamp = rclpy.time.Time(seconds=ts).to_msg()
        msg.header.frame_id = 'map'
        msg.height = 1
        msg.width = n
        msg.is_bigendian = False
...
def main(args=None):
    ...
    if carla_lidar_publisher.vehicle is not None:
        try:
            carla_lidar_publisher.run(repetitions=1)
        finally:
